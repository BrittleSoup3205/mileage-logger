...
4. If an authenticated new device has no stored Mileage Logger state and cloud records already exist, its first sync is pull-only: the app creates a local state shell, applies the cloud records, and uploads no records during that bootstrap.

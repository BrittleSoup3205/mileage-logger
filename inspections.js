  ...
  function inspectionPreviewMarkup(model) {
    ...
    return `
      ...
      <div class="inspection-preview-body">
        ...
        ${model.loads.length ? `<section class="inspection-preview-section"><h3>Vendor loads</h3><div class="inspection-preview-list">${model.loads.map((load) => `<article class="inspection-preview-item"><strong>${escapeHTML(load.identifier)} â€¢ ${escapeHTML(load.status)}</strong>${load.notes ? `<p>${escapeHTML(load.notes)}</p>` : ""}${load.deficiencyFollowUp ? `<p><strong>Deficiency / follow-up:</strong> ${escapeHTML(load.deficiencyFollowUp)}</p>` : ""}${load.photos.length ? `<p><strong>Photos:</strong> ${escapeHTML(load.photos.join(" | "))}</p>` : ""}</article>`).join("")}</div></section>` : ""}
        ${model.followUps.length ? `<section class="inspection-preview-section"><h3>Follow-ups</h3><div class="inspection-preview-list">${model.followUps.map((item) => `<article class="inspection-preview-item"><strong>${escapeHTML(item.action)} â€¢ ${escapeHTML(item.status)}</strong>${item.responsibleParty || item.dueDate ? `<p>${escapeHTML([item.responsibleParty, item.dueDate].filter(Boolean).join(" â€¢ "))}</p>` : ""}</article>`).join("")}</div></section>` : ""}
        <section class="inspection-preview-section"><h3>Inspection photos (${model.photos.length})</h3>${model.photos.length ? `<div class="inspection-preview-photos">${model.photos.map((photo, index) => `<figure class="inspection-preview-photo"><div class="inspection-photo-loading" data-preview-photo="${escapeHTML(photo.id || "")}">Loading photoâ€¦</div><figcaption>${escapeHTML(photoFigureCaption(photo, index + 1))}</figcaption>${photo.name ? `<small>Filename: ${escapeHTML(photo.name)}</small>` : ""}</figure>`).join("")}</div>` : `<p class="muted">No inspection-specific photos are attached.</p>`}</section>
      </div>

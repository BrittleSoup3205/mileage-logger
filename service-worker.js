const CACHE_NAME = "mileage-logger-report-fixes-v115";
...
const TRIP_LOG_DESKTOP_ASSET = "./trip-log-desktop.js?v=responsive-log-6";
...
const INDEX_ASSET = "./index.html?v=report-fixes-41";
...
self.addEventListener("fetch", (event) => {
  ...
  // Never serve authenticated cloud/API responses from the app shell cache.
  // The previous cache-first behavior allowed a device to compare itself to a
  // stale Supabase snapshot and incorrectly report SYNCED. Cross-origin data
  // requests are network-only; local/offline app data remains in localStorage
  // and IndexedDB.
  ...
  if (requestUrl.pathname.endsWith("/trip-log-desktop.js")) {
    event.respondWith((async () => {
      try {
        const response = await fetch(event.request, { cache: "reload" });
        const cache = await caches.open(CACHE_NAME);
        ...
        return await caches.match(event.request) || await caches.match(TRIP_LOG_DESKTOP_ASSET) || new Response("", { status:503 });
      }
    })());
    return;
  }
